...
type Car struct {
	...
	// ManufactureDate holds the value of the "manufacture_date" field.
	...
	for i := range columns {
		...
		case car.FieldManufactureDate:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field manufacture_date", values[i])
			} else if value.Valid {
				c.ManufactureDate = value.Time
			}
		case car.FieldCost:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field cost", values[i])
			} else if value.Valid {
				c.Cost = value.Float64
			}
		}
	}
	return nil
...
func (c *Car) String() string {
	...
	builder.WriteString(", manufacture_date=")
	builder.WriteString(c.ManufactureDate.Format(time.ANSIC))
	builder.WriteString(", cost=")
	builder.WriteString(fmt.Sprintf("%v", c.Cost))
	builder.WriteByte(')')

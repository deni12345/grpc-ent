...
func (cc *CarCreate) SetManufactureDate(t time.Time) *CarCreate {
	cc.mutation.SetManufactureDate(t)
	return cc
}

// SetCost sets the "cost" field.
func (cc *CarCreate) SetCost(f float64) *CarCreate {
	cc.mutation.SetCost(f)
	return cc
}
...
func (cc *CarCreate) check() error {
	...
	if _, ok := cc.mutation.ManufactureDate(); !ok {
		return &ValidationError{Name: "manufacture_date", err: errors.New(`ent: missing required field "Car.manufacture_date"`)}
	}
	if _, ok := cc.mutation.Cost(); !ok {
		return &ValidationError{Name: "cost", err: errors.New(`ent: missing required field "Car.cost"`)}
	}
	return nil
}
...
func (cc *CarCreate) createSpec() (*Car, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.ManufactureDate(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeTime,
			Value:  value,
			Column: car.FieldManufactureDate,
		})
		_node.ManufactureDate = value
	}
